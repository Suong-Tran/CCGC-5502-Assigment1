...
resource "azurerm_virtual_machine_extension" "windows-vme" {
  name                = "${var.windows_name}-vme"
  virtual_machine_id   = azurerm_windows_virtual_machine.windows-vm.id
  publisher            = var.windows_vme["publisher"]
  type                 = var.windows_vme["type"]
  type_handler_version = var.windows_vme["type_handler_version"]
